...
class TuringMachineApp():
    def __init__(self):
        ...
        self.templates_directory = os.path.join(self.current_directory, "default_templates")
    ...
    def create_menus(self):
        ...
        file_menu.add_command(label=i18n.t("open"), command=lambda: import_code(self))
        file_menu.add_command(label=i18n.t("config_editor"), command=self.open_config_editor)
        ...
        templates_menu.add_command(label=i18n.t("generate"), command=lambda: generate_template(self))
        load_templates_menu = tk.Menu(tearoff=0)
        templates_menu.add_cascade(label=i18n.t("load"), menu=load_templates_menu)
        load_templates_menu.add_command(label=i18n.t("invert_value"), command=lambda: import_code(self, os.path.join(self.templates_directory, "invert_values.ptm")))
        load_templates_menu.add_command(label=i18n.t("add_one"), command=lambda: import_code(self, os.path.join(self.templates_directory, "add_one_binary.ptm")))
        load_templates_menu.add_command(label=i18n.t("remove_one"), command=lambda: import_code(self, os.path.join(self.templates_directory, "remove_one_binary.ptm")))
        ...
        main_menu.add_command(label=i18n.t("help"), command=lambda: subprocess.Popen([os.path.join(self.current_directory, "documentation.pdf")], shell=True))
        main_menu.add_command(label="Github", command=lambda: webbrowser.open("https://github.com/Yoween/PythonTuringMachine"))
    ...
    def create_execution_frame(self):
        ...
        for coordinates in self.cell_coordinates.values():
            self.draw_cell(canvas, coordinates[0], coordinates[1], 6, coordinates[2])

        start_button = tk.Button(self.execution_frame, text=i18n.t("start"), command=lambda: execute_code(self, self, self.input_data, slow_var.get()))
        start_button.grid(row=10, column=0, padx=10, sticky='nesw')
        clear_button = tk.Button(self.execution_frame, text=i18n.t("clear"), command=lambda: clear_tape(self, self))
        clear_button.grid(row=10, column=1, sticky='nesw')
        left_button = tk.Button(self.execution_frame, text=i18n.t("left"), command=lambda: move(self, self, "<"))
        left_button.grid(row=10, column=2, sticky='nesw')
        right_button = tk.Button(self.execution_frame, text=i18n.t("right"), command=lambda: move(self, self, ">"))
        ...
        slow_var = tk.IntVar()
        slow_checkbutton = tk.Checkbutton(self.execution_frame, text=i18n.t("slow"), variable=slow_var, offvalue=0, onvalue=1)
        slow_checkbutton.grid(row=11, column=0, padx=10, sticky='nesw')
        b_button = tk.Button(self.execution_frame, text="b", bg=self.color_blank, command=lambda: write(self, self, "b"))
        b_button.grid(row=11, column=1, sticky='nesw')
        zero_button = tk.Button(self.execution_frame, text="0", bg=self.color_zero, command=lambda: write(self, self, "0"))
        zero_button.grid(row=11, column=2, sticky='nesw')
        one_button = tk.Button(self.execution_frame, text="1", bg=self.color_one, command=lambda: write(self, self, "1"))
    ...
    def create_state_frame(self):
        ...
        for i in range(5):
            self.__dict__[f"label_{state_names[i]}"] = tk.Label(self.scroll_frame, text=f"{i18n.t(state_names[i])}", relief=tk.RIDGE)
            self.__dict__[f"label_{state_names[i]}"].grid(row=0, column=i, sticky='nesw')
    ...
    def add_new_state(self, instructions):
        ...
        while a > 0:
            if not f'label_state{x}' in self.__dict__.keys():
                for i in range(3):
                    self.__dict__[f"label_state{x}"] = tk.Label(self.scroll_frame, text=f"{x}")
                    self.__dict__[f"label_state{x}"].grid(row=3 * x - 1, column=0, sticky='nesw')
                    tk.Label(self.scroll_frame, text='||').grid(row=3 * x, column=0, sticky='nesw')
                    tk.Label(self.scroll_frame, text='||').grid(row=3 * x - 2, column=0, sticky='nesw')

                    self.__dict__[f"label_read{x}_{symbols[i]}"] = tk.Label(self.scroll_frame, text=f"{symbols[i]}")
                    self.__dict__[f"label_read{x}_{symbols[i]}"].grid(row=3 * x - 2 + i, column=1, sticky='nesw')

                    self.__dict__[f"label_write{x}_{symbols[i]}"] = tk.Label(self.scroll_frame, text=f"{instructions[symbols[i]][0]}")
                    self.__dict__[f"label_write{x}_{symbols[i]}"].grid(row=3 * x - 2 + i, column=2, sticky='nesw')

                    self.__dict__[f"label_move{x}_{symbols[i]}"] = tk.Label(self.scroll_frame, text=f"{instructions[symbols[i]][1]}")
                    self.__dict__[f"label_move{x}_{symbols[i]}"].grid(row=3 * x - 2 + i, column=3, sticky='nesw')

                    self.__dict__[f"label_new_state{x}_{symbols[i]}"] = tk.Label(self.scroll_frame, text=f'{instructions[symbols[i]][2]}')
                    self.__dict__[f"label_new_state{x}_{symbols[i]}"].grid(row=3 * x - 2 + i, column=4, sticky='nesw')
